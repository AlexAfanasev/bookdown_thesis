...
```{r, cointegration-analysis-plot-covariates, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Cointegration analysis: Visualization of exogenous covariates", fig.width=6, fig.height=6.75, warning=FALSE}
library(tidyverse)
library(gridExtra)
...
The following algorithm descriptions are taken from [@pomp_article, pp. 7-10].
...
\begin{algorithm}
  ...
  \nl \For{$n = 1$ \To{} $N$}{%
    ...
    \nl Compute conditional log likelihood: $\hat{\ell}_{n|1:n-1}(\theta) = \ln\Big(\frac{1}{J}\sum^J_{m=1}w(n, m)\Big)$\;
  }
  \KwOut{Log likelihood estimate: $\hat{\ell}(\theta) = \sum_{n=1}^N\hat{\ell}_{n|1:n-1}(\theta)$ \newline
...
\begin{algorithm}
  ...
  \label{alg:pmmh}
    ...
    Starting parameter $\theta^{(0)}$ \newline
    Prior $\pi(\theta)$ \newline
    Data $y^*_{1:N}$ \newline
    Number of particles $J$ \newline
    Number of iterations $M$
    }
  \nl Initialization: Compute $\hat{\ell}^{(0)} = \hat{\ell}(\theta^{(0)})$ using particle filter with $J$ particles\;
  \nl Draw random trajectory $x^{(0)}_{0:N}$ obtained from particle filter \;
  \nl \For{$m = 1$ \To{} $M$}{%
    \nl Draw a parameter proposal $\theta^*\sim q(\theta^*|\theta^{(m-1)})$\;
    \nl Compute $\hat{\ell}^* = \hat{\ell}(\theta^*)$ using particle filter with $J$ particles\;
    \nl Draw random trajectory $x^{*}_{0:N}$ obtained from particle filter \;
    \nl Compute acceptance probability\; 
    \begin{equation*}
    \alpha(\theta^{*}|\theta^{(m-1)}) = \min\Bigg\{1, \frac{\text{exp}(\hat{\ell}^*)\pi(\theta^*)q(\theta^{(m-1)}|\theta^*)}{\text{exp}(\hat{\ell}^{(m-1)})\pi(\theta^{(m-1)})q(\theta^*|\theta^{(m-1)})}\Bigg\}
    \end{equation*}
