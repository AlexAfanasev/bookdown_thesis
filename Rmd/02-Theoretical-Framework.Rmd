...
This chapter will present a brief description of the basic structure of partially observed Markov processes. Furthermore relevant algorithms for performing likelihood based and bayesian inference will be introduced and their disadvantages and advantages elaborated. However, the content of this thesis will be limited to the case of discrete non-Gaussian and nonlinear state space models. The content of this chapter will be based on the literature of [@pomp_article; @doucet_inference; @doucet_johansen_tutorial; @cappe_smc].
...
As illustrated in [@pomp_article], let $\theta \in R^p$ be the $p$-dimensional vector of model parameters and $\{X(t;\;\theta), t\in T\}$ be the latent state process with $T \subset \mathbb{N}$ and $X(t;\;\theta) \in R^q$. The times at which $X(t;\;\theta)$ is partially observed are denoted $\{t_i \in T, i=1, ..., N\}$ with $t_0 \leq t_1 < t_2 < ... < t_N$ and $t_0 \in T$ being the initial time. Furthermore, the following representation is set: $X_i = X(t_i; \theta)$ and $X_{i:j} = (X_i, X_{i+1}, ..., X_j)$. Besides that, one observes the latent state process by way of a so called observation process $Y_{1:N}$ with $Y_n \in R^r$.
...
- refer to Pitt and shephard? (first reference for particle filter?)

- rewrite algorithms for description?(how to get filtering value for state)
...
Unfortunately, the stochasticity of the likelihood estimate creates issues for most optimizers, as the ones defined in the \texttt{R} \texttt{optim} function, which assume that the objective function is evaluated deterministically. This creates issues since the analytical properties of the target function (convexity, boundedness, smoothness) are often paramount.

Firstly, the particle filter returns just an estimate of the likelihood and no information about the derivative. Approximating the derivative via finite differencing is not promising. Since the likelihood function is already a noisy estimate, one can expect the derivative estimates to be even noisier. Hence, a derivative-free optimization algorithm has to be chosen. \texttt{R}'s \texttt{optim} default method, Nelder-Mead, can be used for this case.

Note that repediately applying the Nelder-Mead procedure will yield different results due to the stochasticity of the likelihood estimate. Futhermore, numerically approximating the hessian via finite difference methods in order to get the fisher information is also not feasible and hence confidence intervals can not be obtained.

Therefore, a better choice in
this case is to use stochastic optimizers such as the iterated filtering method
that we present in the section below

For the simulation approach:
We are concerned with h from a probabilistic (rather than analytical) 
point of view

## Bayesian Inference - Particle Marginal Metropolis-Hastings

Bayesian inference relies on the idea that the observed data updates the formulated prior believe. Hence, using a prior $p(\theta)$ on the unknown parameter $\theta$ inference is based on the joint posterior density $p(x_{0:T}, \theta|y^*_{1:T})$, [@doucet_inference]. This thesis is limited will be limited to the usage of the Particle Marginal Metropolis-Hastings (PMMH) algorithm.



- credible intervals
- algorithms (MCMC, PMCMC)
