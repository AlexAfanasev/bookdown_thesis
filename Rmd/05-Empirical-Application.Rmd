...
For the model comparison the Deviance information criterion (DIC) will be used. Similar to the Akaike information criterion (AIC) and the Bayesian information criterion (BIC), the model with the lower DIC should be preferred. 

The deviance $D(\theta)$ is defined as
$$D(\theta) = -2\ell(\theta) + C$$
with $\ell(\theta$ being the log likelihood for the likelihood function defined in Equation \@ref(eq:pomp-likelihood). $C$ is a constant that cancels out in the model comparison and therefore it can be discarded. 
