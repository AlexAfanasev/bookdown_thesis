...
\end{equation}
A low dimensional vector autoregressive (VAR) model of order $1$ will be used for the objective expectations $\mathbb{\tilde{E}}_t$ conditional on information available at $t$. This approach was first proposed by @campbell_shiller_paper. The VAR model will be comprised of the PD $\eta_t$, excess log dividend growth $\Delta d_t^e$, excess log return $r_t^e$ and inflation $\pi_t$.
...
As visible, the state estimates from the PMMH approach and the particle filter vary, with the particle filter based estimate capturing structural changes later than the PMMH based estimate.

## Cointegration analysis

The connection between economic fundamentals and financial markets will be studied by inspecting possible economic factors that drive the latent mean of the PD. @h2_paper have found three long-term determinants of the latent mean of the PD. The three factors that form a cointegration relationship with the time-varying mean $\tilde{\eta}_t$ are: consumption risk, the demographic structure of the population and the dividend payout policy of firms.
...
for the 40–44, "USPOP49Y" for the 45–49 and "USPOPTO" for the total US population). The middle-aged to young ratio has been computed as the ratio of 40-49 to the 20-29 year old agents. Quarterly data between the end of consecutive years has been obtained by applying linear interpolation. 
...
Firstly, it is possible that the state process is misspecified and another specification, possibly already including covariates, is more suitable. Additionally, the impact of the missing dividend payout policy factor used by @h2_paper is not accounted here. Moreover, it is possible that the cointegration relationship is only present for data with lower frequency than quarterly. Furthermore, different data sources and a shorter time frame have been used. Finally, @h2_paper used the filtered latent mean from the particle filter whereas here the latent mean has been estimated using the PMMH approach. Both approaches differ in which conditional expectation they are targeting, as described in the [particle filtering][Particle filtering] and [Bayesian inference][Bayesian inference] sections.
...
The model extension (**Covariate Model**) describes a possible long run equilibrium relationship that could not be detected within the previous [cointegration analysis][Cointegration analysis],
\begin{equation}
\begin{split}
...
Figure \@ref(fig:pmmh-estimation-figure) displays the estimated latent state for the model extension and the model with random walk state process. Both latent state estimates are moving closely together with the random walk model being smoother and the covariate extension model having more spikes. 
...
Hence, $p_d$ is half the estimated variance of the deviance from the posterior samples $\theta$. Table \@ref(tab:pmmh-model-comparison) depicts the model comparison results. The log likelihood of $\hat\theta$ has been computed using $10$ runs of the particle filter with $J = 1000$ particles and averaging the resulting log likelihoods. As visible, measured by the DIC the random walk model is superior.
