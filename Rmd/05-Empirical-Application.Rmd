...
The empirical application will be conducted on quarterly data for the S&P 500 starting at the 31st of December 1970 up until the 30th of September 2020. Price and dividend data for the S&P 500 have been obtained. Additionally, the 10-Year Treasury Constant Maturity Rate has been used for the risk free rate $r^f_t$ and the inflation rate $\pi_t$ has been derived from the cosumer price index. These time series are retrieved from Robert J. Shiller's website [@shiller_website]
...
```
The model including all covariates has no significant cointegration relationship based on a $10\%$ singificance level. The statistic of $-3.07$ is larger than the critical value $-4.41$ at the $10\%$ level, obtained from the surface regression approach by [@cointegration_critical_values]. 
...
Overall the cointegration analysis has been applied using the estimated latent mean of the price-dividend ratio based on the random walk state process. Contrary to the results presented in [@h2_paper] a cointegration relationship could no be established. The reasons could be the missing dividend payout policy factor or that the cointegration relationship is only present at an annual frequency.

## State process extension with covariates

This chapter will continue the analysis of possible exogenous covariates effecting the latent process. Therefore, 3 additional state process specifications will be tested and compared with the random walk specification.
