...
Three additional models with different state process specifications will be tested and the results will be compared with the random walk model. Each model will contain the previously described covariates. Firstly, each model extension and the estimation results will be presented. Then the latent state estimates will be compared and model comparison performed.

Common similarities between all approaches will be that the same parameter transformations as in the [random walk state process][Random walk state process] chapter will be applied. Hence, the hyperparameters $\varsigma_\epsilon$ and $\varsigma_u$ will be used. Moreover, all PMMH approaches will be run in parallel using 3 independent Markov chains, $J = 1000$ particles and $M = 20000$ iterations.

The first model will describe a long run equilibrium relationship that could not be detected within the [cointegration analysis][Cointegration analysis] chapter
\begin{equation}
\begin{split}
...
$$
Moreover, the following proposal distribution is applied
$$\theta_i^{*}\sim \mathcal{N}(\theta_i^{(m-1)}, 0.015^2)$$

Similar to the random walk state process a flat prior specification will be used. Figure 5.1 displays the trace plots for each parameter and each Markov chain. Additionally, again the first 5000 iterations have been dropped. Furthermore, Figure 5.2 displays the Gaussian kernel density estimates. 

<!-- describe next extension (AR errors model ) -->
<!-- describe next extension ARDL model -->
<!-- At the end: Table ... summarizes the estimation results for all model extensions -->
<!-- show trace & density -->
<!-- show comparison state process estimation -->
