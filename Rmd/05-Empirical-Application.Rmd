...
\end{split}
\end{equation}
For the PMMH procedure $3$ independent Markov chains with $M = 50000$ iterations and $J = 1000$ particles will be run and the starting parameter $\theta_0$ will be set to 
$$
...
```{r, basic-model-state-estimates, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Random walk state model: Filtered latent state", fig.width=6, fig.height=3.5, warning=FALSE}
traj <- rbind(
...
```{r, cointegration-analysis-plot-covariates, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Cointegration analysis: Visualization of exogenous covariates", fig.width=6, fig.height=5, warning=FALSE}
traj <- rbind(
  result[[1]]@filter.traj[1, 10000:50000, ], 
  result[[2]]@filter.traj[1, 10000:50000, ],
  result[[3]]@filter.traj[1, 10000:50000, ]
...
Test statistics for the adjustment coefficient $\alpha$ are used for testing a possible cointegration relationship. The model including all covariates has no significant cointegration relationship based on a $10\%$ significance level. The statistic of `r round(cointegration_model_1$coefficients[2, 3], 3)` is larger than the critical value $-4.41$ at the $10\%$ level, obtained from the surface regression approach by [@cointegration_critical_values]. 

Furthermore, the test has been repeated by removing the insignificant covariates $cr_t$, $fr_t$ and $gdp_t$. Again the cointegration relationship is insignificant at a $10\%$ level. However, the test statistic `r round(cointegration_model_2$coefficients[2, 3], 3)` is larger but relatively close to the critical value $-3.04$. 
...
\end{split}
\end{equation}
The parameter $\phi$ will be restricted for stationarity in the error process, $\left| \phi \right| < 1$. Additionally, the initial value $v_0$ will be set to zero. Similar starting values as in the previous setup are applied
$$
...
\end{split}
\end{equation}
Compared to model \@ref(eq:ar-error-model-state-process) the state process of the ARDL model is linear in parameters. The starting values for this approach are derived by obtaining the corresponding parameter estimates of an ARDL model specification of the complete single equation error correction model from Table \@ref(tab:cointegration-analysis-study). Furthermore, the initial value $\tilde{\eta_0}$ will be added as a necessary parameter for estimation.
$$
\theta_0 = \begin{pmatrix}\beta_0^{(0)} \\ \beta_1^{(0)} \\ \beta_2^{(0)} \\ \beta_3^{(0)} \\ \beta_4^{(0)} \\ \beta_5^{(0)} \\ \tilde{\eta_0} \\ \psi \\ \varsigma_\epsilon^{(0)} \\ \varsigma_u^{(0)} \end{pmatrix} = \begin{pmatrix} 2.75 \\ 0.18 \\ 1.86 \\ 0.75 \\0.69 \\ -0.15 \\ 3.5 \\ \text{atanh}(0.95) \\ \ln(0.05) \\ \ln(0.05) \end{pmatrix}
$$
Here the variance for the proposal distribution will be different for the parameters $\tilde{\eta_0}, \psi, \varsigma_u, \varsigma_\epsilon$ with $\lambda = 0.015^2$ and for the other parameters $\lambda = 0.001^2$.
...
```{r, pmmh-estimation-result, eval=FALSE, echo=FALSE}
library(tidyverse)

model_1 <- lapply(
  readRDS(
    here::here("data", "results", "model_study_result_static_model.rds")
  ), function(x){x@traces[5000:20000, ]}
)
...
model_2 <- do.call(rbind, model_2)

result_table <- data.frame(
  model_1_mean = c(
    NA, mean(model_1[, 3]), mean(model_1[, 4]), mean(model_1[, 5]), 
    mean(model_1[, 6]), mean(model_1[, 7]), mean(model_1[, 8]), 
    NA, mean(exp(model_1[, 9])), mean(exp(model_1[, 10]))
  ),
  model_1_lower = c(
    NA, quantile(model_1[, 3], probs = 0.025), 
    quantile(model_1[, 4], probs = 0.025), 
    quantile(model_1[, 5], probs = 0.025), 
    quantile(model_1[, 6], probs = 0.025), 
    quantile(model_1[, 7], probs = 0.025), 
    quantile(model_1[, 8], probs = 0.025), 
    NA,
    quantile(exp(model_1[, 9]), probs = 0.025), 
    quantile(exp(model_1[, 10]), probs = 0.025)
  ),
  model_1_upper = c(
    NA, quantile(model_1[, 3], probs = 0.975), 
    quantile(model_1[, 4], probs = 0.975), 
    quantile(model_1[, 5], probs = 0.975), 
    quantile(model_1[, 6], probs = 0.975), 
    quantile(model_1[, 7], probs = 0.975), 
    quantile(model_1[, 8], probs = 0.975), 
    NA,
    quantile(exp(model_1[, 9]), probs = 0.975), 
    quantile(exp(model_1[, 10]), probs = 0.975)
  ),
  model_2_mean = c(
    mean(tanh(model_2[, 5])), mean(model_2[, 6]), 
    mean(model_2[, 7]), mean(model_2[, 8]), mean(model_2[, 9]),
    mean(model_2[, 10]), mean(model_2[, 11]), NA, mean(exp(model_2[, 4])), 
    mean(exp(model_2[, 3]))
  ),
  model_2_lower = c(
    quantile(tanh(model_2[, 5]), probs = 0.025), 
    quantile(model_2[, 6], probs = 0.025), 
    quantile(model_2[, 7], probs = 0.025), 
    quantile(model_2[, 8], probs = 0.025), 
    quantile(model_2[, 9], probs = 0.025), 
    quantile(model_2[, 10], probs = 0.025), 
    quantile(model_2[, 11], probs = 0.025), 
    NA,
    quantile(exp(model_2[, 4]), probs = 0.025), 
    quantile(exp(model_1[, 3]), probs = 0.025)
  ),
  model_2_upper = c(
    quantile(tanh(model_2[, 5]), probs = 0.975), 
    quantile(model_2[, 6], probs = 0.975), 
    quantile(model_2[, 7], probs = 0.975), 
    quantile(model_2[, 8], probs = 0.975), 
    quantile(model_2[, 9], probs = 0.975), 
    quantile(model_2[, 10], probs = 0.975), 
    quantile(model_2[, 11], probs = 0.975), 
    NA,
    quantile(exp(model_2[, 4]), probs = 0.975), 
    quantile(exp(model_1[, 3]), probs = 0.975)
  ),
  model_3_mean = c(
...
```{r, pmmh-model-comparison, eval=FALSE, echo=FALSE}
model_1 <- lapply(
  readRDS(
    here::here("data", "results", "model_study_result_static_model.rds")
  ), function(x){x@traces[5000:20000, ]}
...
result_table <- data.frame(
  num_params = c(3, 8, NA, NA),
  lik = c(mean(rw_model[, 1]), mean(model_1[, 1]), NA, NA),
  bic = c(
    -2*mean(rw_model[, 1]) +  log(N)*3, 
    -2*mean(model_1[, 1]) +  log(N)*8, 
    NA, NA
  )
)

rownames(result_table) <- c("RW Model", "Model 1", "Model 2", "Model 3")
result_table <- round(result_table, 4)
opts <- options(knitr.kable.NA = "")
knitr::kable(
  result_table, 
