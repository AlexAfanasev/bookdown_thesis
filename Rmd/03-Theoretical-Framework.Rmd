...
Within the \texttt{pomp} documentation by @king_pomp a naive approach for estimating the likelihood function is presented. This approach uses Monte Carlo integration based upon the Markov property
\begin{equation}
\begin{split}
...
Moreover, there are further possible improvements that deal with this problem. The parameter tuning can be automated by employing an adaptive Metropolis procedure that uses the previous history of the chain to estimate a covariance matrix for the sampler [@adaptive_mcmc]. Additionally, gradient estimates have been successfully incorporated for generating proposals that are more likely to be accepted [@mala_pmmh1; @mala_pmmh2].
