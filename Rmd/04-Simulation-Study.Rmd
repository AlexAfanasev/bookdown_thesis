...
\end{split}
\end{equation*}
Hence, given that $\left|\phi\right|<1$, the long run relationship between the steady-state latent process, $x_{n} = x_{n-1} = \bar{x}$,  and the steady-state exogenous covariates, $z_{1, n} = z_{1, n-1} = \bar{z}_1$, $z_{2, n} = z_{2, n-1} = \bar{z}_2$, is specified by
...
\end{split}
\end{equation}
For simplicity $d_{n}$ will be handled as an exogenous covariate arising from the following specification
\begin{equation}
\begin{split}
...
\begin{split}
\phi &= \text{tanh}(\psi) \\
\sigma_u &= \text{exp}(\varsigma_u) \\
\sigma_{\epsilon} &= \text{exp}(\varsigma_{\epsilon})
...
\end{align*}
The starting hyperparameters $\psi^{(0)}, \varsigma_{\epsilon}^{(0)}, \varsigma_{u}^{(0)}$ are obtained using the corresponding presented transformations in Equation \@ref(eq:parameter-transformation). As visible, the starting parameters will be close to the true values in order to increase the likelihood of convergence for the Markov chains. 

The $10$ runs of the Nelder-Mead optimization and the corresponding particle filtering for the state estimate took roughly $190$ seconds to complete. In comparison, applying one of the presented PMMH setups took approximately $940$ seconds.
...
```{r, sim-4-plot-1, eval=TRUE, echo=FALSE, fig.align='center', fig.cap="Exemplary simulation: Simulated state and observation processes", fig.width=6, fig.height=2, warning=FALSE}
library(ggplot2)
